...
def get_problems_by_keyword(request):
    ...
    problem_title_query = Problem.objects.filter(title__contains = hit, deadline__gte=datetime.now())
    problem_title_query = problem_title_query.filter(Q(status__exact = "pending") | Q(status__exact = "new")) 
    problem_title_query = problem_title_query.filter(~Q(user=ignored_users.values('ref_user_id')))
    
    problem_desc_query = Problem.objects.filter(desc__contains = hit, deadline__gte=datetime.now())
    problem_desc_query = problem_desc_query.filter(Q(status__exact = "pending") | Q(status__exact = "new")) 
    ...
    problem_tag_query = Problem.objects.filter(tags = tag, deadline__gte=datetime.now())
    problem_tag_query = problem_tag_query.filter(Q(status__exact = "pending") | Q(status__exact = "new")) 
